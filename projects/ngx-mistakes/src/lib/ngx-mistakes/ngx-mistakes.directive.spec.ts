...
import { NgxMistakesModule } from '../ngx-mistakes.module';
import { NgxMistakeDirective } from './ngx-mistake.directive';
...
describe('Directives: ngxErrors, ngxError, when', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [ReactiveFormsModule, NgxMistakesModule],
  ...
  it('should hide all ngxError directives on load', () => {
    const elements = el.queryAll(By.directive(NgxMistakeDirective));
    expect(
  ...
  it('should show ngxError[required] when required is true and dirty', async done => {
    const element = el.queryAll(By.directive(NgxMistakeDirective))[0];
  ...
  it('should show ngxError[minlength | maxlength] when either are true, touched and dirty', async done => {
    const element = el.queryAll(By.directive(NgxMistakeDirective))[1];
    expect(component.form.get('prop').dirty).toBe(false);
